...
// Adapted from Flutter flutter_nfc_hce Plugin
// https://github.com/deokgyuhan/flutter_nfc_hce/blob/master/android/src/main/kotlin/com/novice/flutter_nfc_hce/KHostApduService.kt
...
class HCEService : HostApduService() {
    ...
    private var NDEF_URI = NdefMessage(readNdefMessageFromFile(this)?.let {
        createTextRecord("en", it, NDEF_ID);
    })
    ...
    override fun onCreate() {
        super.onCreate()
        Log.i(TAG, "-> ndefMessage initial value: $NDEF_MESSAGE")
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.hasExtra("content")!!) {
            ...
            Log.i(TAG, "persistMessage == true -> writeNdefMessageToFile call, content: " + content)
            writeNdefMessageToFile(this, content)

            NDEF_URI = NdefMessage(createTextRecord("en", content, NDEF_ID))
    ...
    companion object {
        ...
        fun readNdefMessageFromFile(context: Context): String? {
            ...
            try {
                ...
                Log.i(TAG, "Read a message '"+ ndefMessage +"' from NdefMessage.txt.")
        ...
        fun writeNdefMessageToFile(context: Context, ndefMessage: String) {
            try {
                ...
                Log.i(TAG, "Wrote a message to NdefMessage.txt.")
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
        ...
        fun deleteNdefMessageFile(context: Context) {
            try {
                ...
                Log.i(TAG, "The NdefMessage.txt has been deleted.")
            } catch (e: Exception) {
                e.printStackTrace()
            }
        }
